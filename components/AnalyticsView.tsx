...
export const AnalyticsView: React.FC<Props> = ({ habits, themeColor }) => {
  ...
  if (habits.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center px-6">
        <div className={`w-16 h-16 rounded-full bg-${themeColor}-50 dark:bg-${themeColor}-900/20 flex items-center justify-center mb-4`}>
          <Target className={`text-${themeColor}-500`} size={32} />
        </div>
        <h3 className="text-slate-900 dark:text-slate-100 font-bold text-lg mb-2">Sin datos suficientes</h3>
  ...
  return (
    ...
    >
      {/* Identity Score Card */}
      <motion.div variants={item} className="bg-slate-900 dark:bg-slate-800 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
      ...
      <motion.div variants={item}>
        <h3 className="text-slate-800 dark:text-slate-200 font-bold text-lg mb-4 px-1">Tendencia Semanal</h3>
            ...
            {weeklyStats.map((day, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-2 group">
                  <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-lg relative h-full overflow-hidden flex items-end">
                    <motion.div
                      initial={{ height: 0 }}
                      ...
                      className={clsx(
                        ...
                        percentage === 100 ? `bg-${themeColor}-500` :
                          percentage > 0 ? `bg-${themeColor}-300` : "bg-slate-200 dark:bg-slate-800"
                      )}
                    />
                  </div>
                  <span className={clsx(
                    "text-xs font-bold",
                    day.date === new Date().toISOString().split('T')[0] ? "text-slate-900 dark:text-slate-100" : "text-slate-400 dark:text-slate-600"
                  )}>
                    {day.dayName}
                  </span>
                </div>
      ...
      <motion.div variants={item} className="flex flex-col gap-3">
        <h3 className="text-slate-800 dark:text-slate-200 font-bold text-lg mb-1 px-1">Desglose de Hábitos</h3>
        {habits.map(habit => {
          const bestStreak = calculateBestStreak(habit.completedDates);
          const totalReps = habit.completedDates.length;

          return (
            <div key={habit.id} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-4 shadow-sm flex flex-col gap-3">
              <div className="flex items-center gap-3 mb-1">
                <span className="text-2xl">{habit.icon || '📝'}</span>
                <span className="font-semibold text-slate-800 dark:text-slate-200">{habit.title}</span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-2 flex flex-col items-center text-center">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-lg font-bold text-slate-700 dark:text-slate-300">{bestStreak}</span>
                  </div>
                </div>
                <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-2 flex flex-col items-center text-center">
                  <span className="text-[10px] text-slate-400 font-bold uppercase">Total</span>
                  <span className="text-lg font-bold text-slate-700 dark:text-slate-300">{totalReps}</span>
                </div>
              </div>
            </div>
          );
